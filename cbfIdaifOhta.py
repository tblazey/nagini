...
cbfIdaifOhta.py: Calculates CBF using a 015 water scan and an image-derived input function

Uses model described by Ohta et al, Journal of Cerebral Blood Flow and Metabolism 1996
...
User can set the following options:
	...
	fBound -> Bounds for flow parameter. Default is 10 times the whole brian value.
	kBound -> Bounds for kTwo (clearance from brain to blood). Default is 10 times the whole brian value.
	vBound -> Bounds for vZero (apparent vascular distrubtion volume of tracer). Default is 0 to 1.
	
Produces the following outputs:
	cbf -> Voxelwise map of cerebral blood volume in mL/hg*min
...
argParse.add_argument('-kBound',nargs=2,type=float,metavar=('lower','upper'),help='Bounds for lambda parameter. Default is 10 times whole brain value')
...
for bound in [args.fBound,args.kBound,args.vBound]:
...
nVox = petMasked.shape[0]; fitParams = np.zeros((nVox,7)); noC = 0;
for voxIdx in tqdm(range(nVox)):
	...
	try:
		...
		fitParams[voxIdx,3] = fitVar[0] * np.power(6000.0/args.d[0],2)
		fitParams[voxIdx,4] = fitVar[1] * np.power(60,2)
		fitParams[voxIdx,5] = fitVar[2] * np.power(100 / args.d[0],2)
		...
		fitParams[voxIdx,6] = fitRmsd / np.mean(voxInterp)
...
imgNames = ['flow','kTwo','vZero','flow_var','kTwo_var','vZero_var','nRmsd']
